...
const CharacterContent = ({
  ...
  <div className="space-y-2 min-h-80 p-4 spotty-bg-[#F3B807]">
    <p>
...
const ComicFact = ({ fact }: { fact: string }) => (
  <div className="p-4 spotty-bg-[#F3B807] min-h-24 flex items-center justify-center text-center">
          ...
          <TabsList>
            {syncData.map((item) => (
              <TabsTrigger
                key={item.id}
                value={item.id}
                className="spotty-bg-[#F3B807] font-black text-xs"
              >
                {item.label}
              </TabsTrigger>
            ))}
